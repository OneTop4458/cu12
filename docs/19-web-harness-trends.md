...
Research snapshot: **2026-09-08**. Primary sources were opened and read on that date. This document records repository-specific decisions, not a new product specification or permission to redesign the website. General web improvements remain [plan only](20-ui-ux-improvement-plan.md); the separately requested administrator features have implementation scope.
...
`DESIGN.md` remains the current visual contract. The [UI/UX improvement plan](20-ui-ux-improvement-plan.md) records deferred general-web work and its acceptance criteria. Operational behavior belongs in living specifications and runbooks; this research snapshot should not be used to infer that a planned feature has shipped.
