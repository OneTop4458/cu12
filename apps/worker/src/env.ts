...
const EnvSchema = z.object({
  ...
  PLAYWRIGHT_NAVIGATION_TIMEOUT_MS: z.coerce.number().int().min(10_000).max(300_000).default(120_000),
  PLAYWRIGHT_NAVIGATION_RETRIES: z.coerce.number().int().min(0).max(5).default(2),
  PLAYWRIGHT_NAVIGATION_RETRY_BASE_MS: z.coerce.number().int().min(500).max(60_000).default(5_000),
  ...
  WORKER_RETRY_WAIT_MAX_MS: z.coerce.number().int().min(0).max(3_600_000).default(1_200_000),
  WORKER_WORKFLOW_STARTED_AT_MS: z.coerce.number().int().min(0).optional(),
