...
test("dashboard and admin pages use common topbar without legacy button override", () => {
  ...
  assert.match(css, /\.page-shell \{[\s\S]+?grid-template-columns: minmax\(0, 1fr\);/);
  assert.match(css, /\.topbar \{[\s\S]+?background: var\(--cuk-black\);[\s\S]+?\.topbar-main \{[\s\S]+?width: calc\(100% - 48px\);/);
