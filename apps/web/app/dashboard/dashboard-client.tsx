      ...
      if (action === "AUTOLEARN") {
        const baseMessage = payload.notice
          ?? (payload.deduplicated
            ? "같은 조건의 자동 수강 요청이 이미 있어 현재 작업이 끝난 뒤 이어서 진행돼요."
            : "자동 수강 요청이 접수됐어요. 순서대로 곧 시작돼요.");
        const pendingGuide = "요청이 몰리면 잠시 기다릴 수 있어요. 보통 몇 분 안에 시작됩니다.";
        const mailGuide = "메일 알림에서 '자동 수강 시작/종료 알림 발송'을 켜두면 시작과 종료를 바로 알려드려요.";
        if (payload.dispatched === false || payload.dispatchState === "FAILED" || payload.dispatchState === "NOT_CONFIGURED") {
          setMessage(`${baseMessage} 실행 준비가 조금 지연되고 있어 시작까지 더 걸릴 수 있어요. ${pendingGuide} ${mailGuide}`);
        } else {
        ...
        if (payload.dispatched === false) {
          setMessage(`${baseMessage} 실행 준비 중이라 시작까지 조금 더 걸릴 수 있어요.`);
        } else {
  ...
  return (
    <>
      ...
      <section className="card" id="jobs">
        <h2>자동 수강</h2>
        <p className="muted">동기화된 최신 정보를 기준으로 자동 수강을 실행합니다. 아래 버튼은 지금 바로 수동으로 요청할 때 사용해요.</p>
        <p className="muted text-small">요청 후에는 순서대로 진행되며 잠시 대기할 수 있어요. 보통 몇 분 안에 시작되지만, 요청이 몰리면 더 걸릴 수 있습니다.</p>
        <p className="muted text-small">메일 알림에서 '자동 수강 시작/종료 알림 발송'을 켜면 시작과 종료를 메일로 받아볼 수 있어요.</p>
        <div className="button-row">
