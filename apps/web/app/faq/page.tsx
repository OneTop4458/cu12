...
const FAQ_ITEMS = [
  {
    ...
    answers: [
      ...
      "필요한 경우 동기화 또는 자동학습 요청이 접수되며, 백그라운드 작업이 완료되면 결과와 알림이 반영됩니다. 최초 이용 시에는 기본적으로 포털 인증 후 관리자 승인을 기다립니다. 관리자가 승인 대기를 꺼 둔 동안에는 로그인 시 자동 승인되며, 이후 약관 동의를 완료하면 이용할 수 있습니다.",
    ],
  },
  {
  ...
  },
  {
    ...
    answers: [
      ...
      "메일 알림, 공유대 정기 자동 수강, 퀴즈 자동 풀이 같은 옵션은 로그인 후 회원 설정에서 조정할 수 있습니다. 사이버캠퍼스는 사용자가 직접 요청하며, 추가 인증이 필요한 경우 안내에 따라 인증을 완료합니다.",
    ],
  },
  {
  ...
  },
  {
    ...
    answers: [
      "본 서비스는 동기화, 자동학습, 메일 알림, 백그라운드 작업 처리 등 지속적인 서버 자원을 사용합니다. 운영 가능한 사용자 수와 작업량을 관리하기 위해 관리자 승인 대기를 기본으로 사용합니다.",
      "사용자는 먼저 실제 포털 ID와 비밀번호로 본인 계정임을 검증합니다. 승인 대기가 ON이면 관리자 승인 후 다시 로그인해 계정 연동과 약관 동의를 진행합니다. OFF 동안에는 신규·대기 회원이 인증에 성공하면 자동 승인되어 같은 절차를 이어갑니다.",
    ],
  },
] as const;
