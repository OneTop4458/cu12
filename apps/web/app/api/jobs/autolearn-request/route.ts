...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (mode !== "ALL_COURSES" && !body.lectureSeq) {
      return jsonError("선택 강의 모드에서는 lectureSeq가 필요합니다.", 400);
    }
    ...
    if (provider === "CYBER_CAMPUS") {
      ...
      const isApprovalRequired = result.kind === "APPROVAL_REQUIRED";
      const hasDispatchState = result.kind === "QUEUED" || result.kind === "APPROVAL_PENDING";

      return jsonOk({
        provider,
        jobId: result.jobId,
        status: result.status,
        deduplicated: hasDispatchState ? result.deduplicated : false,
        dispatched: hasDispatchState ? result.dispatched : false,
        dispatchState: hasDispatchState ? result.dispatchState : "NOT_APPLICABLE",
        dispatchError: hasDispatchState ? result.dispatchError : null,
        dispatchErrorCode: hasDispatchState ? result.dispatchErrorCode : null,
        notice: result.notice,
        approvalRequired: isApprovalRequired,
        approval: isApprovalRequired ? result.approval : null,
      });
    }
    ...
    const notice = deduplicated
      ? dispatch.state === "SKIPPED_DUPLICATE"
        ...
        : "이전 자동 수강 요청이 지연된 것으로 보여 실행 준비를 다시 시작했습니다."
      : dispatch.dispatched
    ...
    if (normalized) {
      await writeAuditLog({
        ...
        message: "Cyber Campus AUTOLEARN transport error",
